...
def generate_preferences(OUTFILE, order, students, courses, M, tracksData, RNG, probs):
    ...
    for studentID, curriculum in enumerate(order, start=1):
        ranks, optionalNumber = generate_student_ranks(curriculum, courses, M, tracksData, RNG, probs)
...
def main():
    ...
    parc, maxOptional, r = generate_preferences(OUTFILE, order, students, courses, M, tracksData, RNG, probs)

    print(f"Generated {args.n} students ({args.ai} AI, {args.cs} CS) -> {OUTFILE}  [profile={args.profile}]")
