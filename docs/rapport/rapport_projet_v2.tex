...
Dans ce rapport, on qualifie une affectation d'\emph{équitable} si, à contraintes satisfaites, elle (i) garantit un niveau de satisfaction acceptable pour l'ensemble des étudiants (éviter les situations \og très défavorisées \fg{}) et (ii) limite les écarts de satisfaction entre étudiants, tout en restant compatible avec une interprétation simple par les responsables de formation.
\end{definition}
...
\paragraph{Exemple d'instance et d'affectation.}
...
Par cette définition et cet example, nous observons donc que les métriques vectorielles permettent généralement d'attribuer un score à l'affectation d'un étudiant tandis qu'une aggregation de ces métriques (voir Annexe I pour plus de détails) permettent d'évaluer cette notion d'équité.
...
\textbf{À faire.} Finaliser la méthodologie, compléter les expériences, et discuter les résultats obtenus sur des instances réelles.
