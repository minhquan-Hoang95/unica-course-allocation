...
À des fins de comparaison, nous considérons une baseline simple de type \og premier arrivé, premier servi \fg{} (FCFS). En effet, ce système constitue un point de référence utile pour plusieurs raisons (i) il est facile à implémenter, (ii) il est facile à expliquer, (iii) il s'agit du système couramment mis en place. Néanmoins, la méthode d'affectation ne garantit pas une bonne satisfaction globale ni une équité satisfaisante.
...
Avec ce modèle, nous émettons l'hypothèse simplificatrice, mais cohérente, que peu importe le parcours, le nombre minimum d'UEs pour passer un semestre ne change pas. En pratique, cette hypothèse est vérifié, car toutes les UEs sont de 3 crédits ECTS, exceptés les projets DS4H et TER comptant pour 6 crédits ECTS indépendamment des parcours. 
...
Aussi, comme vous l'aurez remarquez, d'autres objectifs sont également implémentés ; en tout une dizaine minimisant ou maximisant certains critères le sont. Dans notre cas, nous avons présenté les plus simples et compréhensible, mais d'autres pourraient êtres plus adaptés. Dans l'annexe II, vous retrouverez l'intégralité des objectifs implémentés (dans OPL, ils sont implémentés sous leur forme de rangs non facultatifs).
...
Bien que nous ayons la capacité de comparer des affectations en se basant uniquement sur des métriques, cela peut poser des difficultés. En effet, en plus du temps demandé pour l'interprétation et la comparaison de celles-ci entre plusieurs affectations, il est aisé de faire des erreurs d'inattention ou d'interprétation menant au choix d'une affectation sous-optimale.
...
Pour cela, l'étudiant en charge de cette partie à mis en place divers graphiques sur les différentes métriques vectorielles : bar plot, box plot, scatter plot, etc. 
Ces graphique vise à mesurer rapidement et efficacement les aspects suivants :
...
Le graphique le plus parlant (même si non abouti) reste néanmoins celui-ci :
\begin{figure}[H]
    \includegraphics[width=\textwidth]{img/heatmapMedian.png}
    \caption{Heatmap des affectations}
\end{figure}

Sur celui-ci, les UEs obligatoires sont retirées, car elles possèdent le meilleur rang possible quoi qu'il arrive. De plus, les deux parcours n'ayant pas nécessairement le même nombre d'UEs obligatoires, nous séparons les parcours pour avoir un résultat plus représentatif. Il ne reste donc plus que le rang normalisé à examiné. 
Ainsi, nous pouvons comparer deux affectations à vu d'œil juste en regardant à quel point l'une est plus "verte" que l'autre. Intuitivement, nous prendrons l'affectation ayant le plus d'étudiant satisfait. À noter que cette visualisation peut changer, en n'affichant qu'un parcours, en ne considérant qu'un parcours en particulier, en remettant les UEs obligatoires ou en changeant l'ordre qui par défaut se fait par la médiane des rangs non facultatifs en ordre croissant.
...
À ces fins, regrouper les métriques vectorielles, certains indicateurs et graphiques, dans un tableau de bord, pourrait être également une bonne option. Nous pouvons imaginer assez facilement quelque chose ainsi :
\begin{figure}[H]
...
\end{figure}
Où nous serions capable d'accéder facilement par un filtre à toutes les métriques agrégées, par un histogramme au remplissage des UEs/leurs saturations ; par des tests d'hypothèses sur l'ordre, le parcours ou le nombre d'UE choisies et l'indice de Gini si une affectation présente des propriétés non souhaitables sur des métriques choisies (indiqué par des pastilles de couleurs vert/jaune/rouge par exemple), les paramètres de notre affectation, ainsi que la heatmap de l'affectation des étudiants aux UEs en fonction du parcour et triée par la métrique choisie.

Ce tableau de bord, n'a malheureusement pas vu le jour en raison de contraintes de temps.

\section{Conclusions}
En conclusion, ce projet vise à proposer un mécanisme d'affectation plus << satisfaisant >> que le système en place : << premier arrivé, premier servi >>, le tout en restant interprétable (au mieux explicable), en introduisant la notions d'équité et en la différenciant avec d'autres notions tels que l'optimalité ou l'intégrité.

Pour cela, le format de donnée à d'abord été formalisé tout comme les l'ont été les contraintes ; les notions d'équité et d'optimalité ont été fortement étudié afin de déterminer les métriques et objectifs les plus proche de ce que l'on juge d'affectation <<satisfaisante>>.
Dans un second temps, un modèle linéaire, pour lequel l'objectif est sélectionnable, a été créé et utilisé au sein d'un solveur exact (OPL/CPLEX). Les affectations sortantes (au format CSV) sont ensuite analysé à l'aide des métriques pré-citées et visualisés afin de soutenir une décision d'affectation finale pour les étudiants. 
...
\paragraph{Contributeurs} Les contributes aux projets sont les suivants : 
