...
def create_occupancy_column():
    df_cours["occupancy"] = (df_cours["spots"] - df_cours["freeSpots"]) / df_cours["spots"]
    return df_cours
...
def create_rank_distance_comparison(df_student):
    ...
    fig = make_subplots(rows=1, cols=3,
                        subplot_titles=("MinRankDistance", "MaxRankDistance", "correctedMaxRankDistance"))

    fig.add_trace(
        go.Box(y=df_student["minRankDistance"], name="MinRankDistance"),
        row=1, col=1
    )

    fig.add_trace(
        go.Box(y=df_student["maxRankDistance"], name="MaxRankDistance"),
        row=1, col=2
    )

    fig.add_trace(
        go.Box(y=df_student["correctedMaxRankDistance"], name="correctedMaxRankDistance"),
        row=1, col=3
    )
...
def course_pop_capacity_use():
    fig = px.scatter(df_cours, x="median", y="occupancy", title="Course Capacity vs Occupancy")
