...
PARETO_FUNCTION = Union[Literal[
    "sum","mean","var","std",
    "median","q1","q3","n",
    "firstRank","lastRank","minStudentRank","maxStudentRank",
    "requested","optionalCorrectedRankSum","lastMandatoryOptionalRank","rankedVSrequested",
    "maxRankDistance","minRankDistance","correctedMaxRankDistance","optionalCorrectedQ1",
    "optionalCorrectedQ3","optionalCorrectedMedian","optionalCorrectedMean","optionalCorrectedVar",
    "optionalCorrectedStd", "IQR","optionalCorrectedIQR",
]]
...
def getPossibleParetoExchanges(affectations:pd.DataFrame, 
    ...
    mandatoryLectures = mu.getStudentsMandatoryLecturesFromTrack(courses, mu.getStudentsTrackFromPreferences(preferences))
    ...
    for student1 in preferences.loc[:,"studentID"]:
        ...
        for student2 in preferences.loc[preferences["studentID"]>student1, "studentID"]:
            ...
            canStudent1ExchangeWithStudent2 = np.array_equal(affectationsMatrix[student1-1, mandatoryLecturesIndices[student1-1]], affectationsMatrix[student2-1, mandatoryLecturesIndices[student1-1]])
            if not canStudent1ExchangeWithStudent2: continue

            # Check if student 2 can exchange with student 1 affectation regarding to student 2 mandatory lectures
            canStudent2ExchangeWithStudent1 = np.array_equal(affectationsMatrix[student1-1, mandatoryLecturesIndices[student2-1]], affectationsMatrix[student2-1, mandatoryLecturesIndices[student2-1]])
            if not canStudent2ExchangeWithStudent1: continue
            
            # Does student 1 have more courses than student 2 whishes?
            haveStudent1LessWhishesThanStudent2 = affectationsMatrix[student1-1,:].sum() <= coursesRequestVector[student2-1]
            if not haveStudent1LessWhishesThanStudent2: continue
            
            # Does student 2 have more courses than student 1 whishes
            haveStudent2LessWhishesThanStudent1 = affectationsMatrix[student2-1,:].sum() <= coursesRequestVector[student1-1]
            if not haveStudent2LessWhishesThanStudent1: continue
            ...
            startingRankMatrix = np.array([startStudent1RankMatrix,affectationsMatrix[student2-1,:]*preferencesMatrix[student2-1,:]])
            
            # Same, but after swapping the affectations
            swapRankMatrix = np.array([affectationsMatrix[student2-1,:]*preferencesMatrix[student1-1,:], affectationsMatrix[student1-1,:]*preferencesMatrix[student2-1,:]])
            
            # Filter to get the right metrics (we do not want metrics that keeps other students in memory)
            studentPreferencesFilter = np.logical_or(preferences["studentID"]==student1, preferences["studentID"]==student2)            
            ...
            additionalFunc = mu.getAdditionalCoursesBasedOnStudentsFunction(preferences.loc[studentPreferencesFilter,:], courses, parameter)
            additionalFunc |= mu.getAdditionalPreferencesBasedOnStudentsFunction(preferences.loc[studentPreferencesFilter,:])
...
if __name__ == "__main__":
    ...
    affectations, preferences, courses = mu.getData(mu.DEFAULT_IN_AFFECTATIONS_PATH, mu.DEFAULT_IN_PREFERENCES_PATH, mu.DEFAULT_IN_COURSES_PATH)
    parameters = mu.getParameters(mu.DEFAULT_IN_PARAMETERS_PATH)
    
    # Get the Pareto and possible exchanges
    paretoExchanges, technicalExchanges = getPossibleParetoExchanges(affectations, preferences, courses, parameters, "lastMandatoryOptionalRank", "min", returnTechnicalExchange=True)
    print(f"There is {len(paretoExchanges)} Pareto exchanges possible over {len(technicalExchanges)} possible exchanges. The Pareto exchange density is {len(paretoExchanges)/len(technicalExchanges):.3f}")
