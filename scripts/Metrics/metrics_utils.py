...
def computeBaseVectors(
    rankMatrix: np.ndarray,
...
def getAdditionalCoursesBasedOnStudentsFunction(
    preferences: pd.DataFrame, courses: pd.DataFrame, parameters: dict
) -> dict[str, partial | Callable]:
...
def sort_students(student_metrics, strategy: list, order_statements: list):
    sorted_students = student_metrics.sort_values(
        by=strategy,  # consistency + IQR as a tie-breaker
        ascending=order_statements,
    ).index
...
def createHeatmap(df_colors: pd.DataFrame, name_file, strategy):
    ...
    sns.heatmap(df_colors, cmap="RdYlGn_r", linewidths=0.5, linecolor="gray")

    plt.title("Student Satisfaction Heatmap (" + "".join(strategy) + ")")
    plt.xlabel("Courses")
...
if __name__ == "__main__":
    ...
    df_matrix = pd.DataFrame(
        ...
        columns=affectations.columns[affectations.columns.str.startswith("c")],
    )

    student_metrics = computeBaseVectors(
        rankMatrix,
        getAdditionalPreferencesBasedOnStudentsFunction(preferences)
        | getAdditionalCoursesBasedOnStudentsFunction(preferences, courses, parameters),
        *getAdditionalPostOnStudentsFunction(),
        onStudent=True,
    )
    ...
    strategies = {
        ...
        "mean_IQR": (["optionalCorrectedMean", "optionalCorrectedIQR"], [True, True]),
    }

    for name, (cols, asc) in strategies.items():
        sorted_students = sort_students(student_metrics, cols, asc)
        df_sorted = df_matrix.loc[sorted_students]
        df_normalized = df_sorted.apply(
            lambda row: normalize_row(row, global_max), axis=1
        )
        df_packed = df_normalized.apply(pack_row, axis=1)
        createHeatmap(df_packed, f"visualisation/out/{name}", cols)

    """
    create_split_heatmaps(df_matrix, student_metrics, output_prefix="out/split_heatmap")
    create_split_heatmaps(df_matrix, student_metrics, output_prefix="out/split_optional",
                        exclude_mandatory=True, courses=courses)
    """

    create_split_heatmaps(
        df_matrix,
        student_metrics,
        preferences,
        output_prefix="visualisation/out/split_heatmap",
    )
    create_split_heatmaps(
        df_matrix,
        student_metrics,
        preferences,
        output_prefix="visualisation/out/split_heatmap_optional",
    ...
    create_gini_plot(
        ...
        metric="optionalCorrectedRankSum",
        output_path="visualisation/out/gini_plot.png",
    )

    # col_order = df_colors.mean(axis=0).sort_values(ascending=True).index
    # df_colors = df_colors[col_order]
    # createHeatmap(df_colors)
